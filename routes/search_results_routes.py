...
async def perform_hybrid_search(collection, reqs, output_fields, name=0.8, meta=0.2):
    ...
    results = collection.hybrid_search(
        reqs, rerank, limit=200, output_fields=output_fields
    )
    for result in results[0]:
        processed_results.append(
            {
                "distance": result.distance,
                "Metaphone_Name_After_Sort": result.entity.get(
                    "Metaphone_Name_After_Sort"
                ),
...
async def hybrid_vector_search_for_count(name, title, meta):
    try:
        ...
        output_fields = [
            "Metaphone_Name_After_Sort",
            "Title_Name",
            "Title_Name_After_Sort",
        ]
        final_result_df = await perform_hybrid_search(
            collection,
            reqs,
            output_fields,
            title,
...
def calculate_dynamic_impacts(fuzzy_scores, title):
    ...
    medium_impact = (
        (medium_avg / 100) * 0.5 if n_medium > 0 else 0
    )  # Scale down medium scores
...
async def same_title(title: str = Query(..., description="The name to search for")):
    try:
        ...
        meta_dist = spell_check(
            get_metaphone(title_after_sort.upper()), result["Metaphone_Name_After_Sort"]
        )
        result["fuzzy"] = result["Title_Name_After_Sort"].apply(
            lambda x: fuzz.ratio(title_after_sort.upper(), x)
        )
...
async def similar_sound(title: str = Query(..., description="The name to search for")):
    try:
        ...
        fuzzy_values = resultFLD["fuzzy"].tolist()
        probability = calculate_dynamic_impacts(fuzzy_values, name)
        # resultFDL = resultFDL.reset_index(drop=True)
        print(json.dumps(probability, indent=2))
        return {
            "status": "success",
            "message": f"Titles as same as {name}",
            "isValid": True,
